...
document.addEventListener("click", function (e) {
  if (!e.target.closest(".proc-wrapper")) {
    ...
    document.querySelectorAll(".is-expanded").forEach(b => b.classList.remove("is-expanded"));
  }
});

// ── Dropdowns des plc-row (comportement identique aux procédures) ──
document.querySelectorAll(".plc-row-buttons .proc-wrapper").forEach(wrapper => {
  const parent = wrapper.querySelector(":scope > button");
  const subGroup = wrapper.querySelector(":scope > .proc-sub-group");
  if (!parent || !subGroup) return;
  ...
  parent.addEventListener("click", function (e) {
    ...
    document.querySelectorAll(".is-expanded").forEach(b => {
      if (b !== parent) b.classList.remove("is-expanded");
    });
